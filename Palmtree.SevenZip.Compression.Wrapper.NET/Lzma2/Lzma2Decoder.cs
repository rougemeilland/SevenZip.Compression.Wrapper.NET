...
namespace SevenZip.Compression.Lzma2
{
        ...
        : IDisposable, ICompressCoder, ICompressGetInStreamProcessedSize
    {
        ...
        private Lzma2Decoder(CompressCoder compressCoder, CompressGetInStreamProcessedSize compressGetInStreamProcessedSize)
        {
            _isDisposed = false;
        ...
        /// <summary>
        /// Create an instance of <see cref="Lzma2Decoder"/> with default properties.
        /// </summary>
        /// <param name="contentProperties">
        /// <para>
        /// Set the data that represents the parameters of the compressed data in LZMA2 format.
        /// </para>
        /// <para>
        ...
        /// </returns>
        public static Lzma2Decoder CreateDecoder(ReadOnlySpan<Byte> contentProperties) => CreateDecoder(new Lzma2DecoderProperties(), contentProperties);

        /// <summary>
        /// Create an instance of <see cref="Lzma2Decoder"/>.
        /// </summary>
        /// <param name="properties">
        /// Set a property container object to customize the behavior of the LZMA2 decoder.
        /// </param>
        /// <param name="contentProperties">
        /// <para>
        /// Set the data that represents the parameters of the compressed data in LZMA2 format.
        ...
        public static Lzma2Decoder CreateDecoder(Lzma2DecoderProperties properties, ReadOnlySpan<Byte> contentProperties)
        {
            ...
            var compressSetMemLimit = (CompressSetMemLimit?)null;
            var success = false;
            try
            {
                ...
                if (properties.InBufSize is not null || properties.OutBufSize is not null)
                {
                    ...
                    {
                        // The streamIndex parameter is ignored in the LZMA2 decoder.
                ...
                if (properties.MemUsage is not null)
                {
                    compressSetMemLimit = (CompressSetMemLimit)compressCoder.QueryInterface(typeof(CompressSetMemLimit));
        ...
        /// <summary>
        /// Read the compressed data from the input stream, decode it with LZMA2, and write the uncompressed data to the output stream.
        /// </summary>
        ...
        public static ICompressDecoderStream CreateDecoderStream(
            ...
            => CreateDecoderStream(compressedInStream, new Lzma2DecoderProperties(), uncompressedOutStreamSize, contentProperties, leaveOpen);
        ...
        public static ICompressDecoderStream CreateDecoderStream(
            ISequentialInputByteStream compressedInStream,
            Lzma2DecoderProperties properties,
            UInt64? uncompressedOutStreamSize,
            ReadOnlySpan<Byte> contentProperties,
            Boolean leaveOpen = false)
        {
