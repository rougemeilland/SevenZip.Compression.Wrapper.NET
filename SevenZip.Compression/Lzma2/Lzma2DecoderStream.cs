...
{
    /// <summary>
    /// A class of LZMA2 decoders in virtual stream format that can be read sequentially.
    /// </summary>
    class Lzma2DecoderStream
        : IO.DecoderStream
    {
        /// <summary>
        /// The length of the property embedded in the compressed stream in LZMA2 format.
        /// </summary>
        public const Int32 LZMA_CONTENT_PROPERTY_SIZE = 1;

        private readonly ICompressGetInStreamProcessedSize _compressGetInStreamProcessedSize;

        private bool _isDisposed;

        private Lzma2DecoderStream(ISequentialInStream sequentialInStream, ICompressGetInStreamProcessedSize compressGetInStreamProcessedSize)
        ...
        /// <summary>
        /// Create an instance of <see cref="Lzma2DecoderStream"/>.
        /// </summary>
        ...
        /// </param>
        /// <returns>
        /// The created <see cref="Lzma2DecoderStream"/> object.
        /// </returns>
        ...
        public static Lzma2DecoderStream Create(Stream compressedInStream, Lzma2DecoderProperties properties, ReadOnlySpan<Byte> contentProperties, UInt64? uncompressedOutStreamSize)
        {
        ...
        /// </returns>
        /// <exception cref="ArgumentNullException"><paramref name="compressedInStream"/> or <paramref name="properties"/> is null.</exception>
        public static Lzma2DecoderStream Create(IO.ISequentialInStream compressedInStream, Lzma2DecoderProperties properties, ReadOnlySpan<Byte> contentProperties, UInt64? uncompressedOutStreamSize)
        {
            if (compressedInStream is null)
                throw new ArgumentNullException(nameof(compressedInStream));
            if (properties is null)
        ...
        private static Lzma2DecoderStream Create(SequentialInStreamReader compressedInStreamReader, Lzma2DecoderProperties properties, ReadOnlySpan<Byte> contentProperties, UInt64? uncompressedOutStreamSize)
        {
            ...
            try
            {
                ...
                var decoder = new Lzma2DecoderStream(sequentialInStream, compressGetInStreamProcessedSize);
                success = true;
                return decoder;
            }
            finally
